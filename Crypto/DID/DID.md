...
A **VC (Verifiable Credential)** is the digital equivalent of a physical credential like a passport, driver's license, or university diploma. It's a tamper-evident, cryptographically verifiable digital certificate that makes claims about a subject.

A **VP (Verifiable Presentation)** is the package you create and send to a verifier when you need to prove something about yourself. It's how you present your credentials in a controlled and privacy-preserving way.

| Feature | DID (Decentralized Identifier) | VC (Verifiable Credential) | VP (Verifiable Presentation) |
| :--- | :--- | :--- | :--- |
| Core Concept | The Identity | The Evidence | The Proof |
| Analogy | Your physical body or a unique ID number. | A passport or driver's license in your pocket. | Showing your ID to a bartender to prove age. |
| Primary Function | Uniquely identifies a subject (person, org, device) without a central authority. | Makes specific claims about a subject (e.g., "Name is Alice," "Has Degree"). | Presents claims to a verifier, often with privacy filters (Selective Disclosure). |
| Who Creates It? | The Subject (You) creates it for yourself. | An Issuer (Authority) creates and signs it for you. | The Holder (You) creates it to send to someone else. |
| Key Content | Public Keys, Service Endpoints, Metadata. | Claims (Attributes), Issuer Signature, Expiry Date. | The VC(s) being shown, Holder Signature, Proof of possession. |
| Dependency | Independent. It is the root of trust. | Depends on DID. It is issued *to* a DID and *by* a DID. | Depends on VC. It wraps one or more VCs to present them. |
